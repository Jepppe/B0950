...
'''This section deals with trying to isolate the peak flux densities'''

#Create an array to store the data for the peak intensities

peaksnr = np.zeros(numpul, dtype = {'names': ('Pulse', 'Time', 'Peak SNR'), 'formats': ('int', 'f8', 'f8')})
...
while pulse < numpul:
    peaksnr[pulse]['Pulse'] = pulse + 1
    pulse += 1

#Work out the peak SNRs of pulses
    
rownum = 0
pulse = 0

peaksnr[0]['Time'] = z[0]['Time']
peaksnr[0]['Peak SNR'] = z[0]['SNR']

while rownum < nrows - 1:
    if z[rownum + 1]['Time'] - z[rownum]['Time'] <= 0.253/(2*60*60*24) and z[rownum + 1]['SNR'] > peaksnr[pulse]['Peak SNR']:
        peaksnr[pulse]['Peak SNR'] = z[rownum + 1]['SNR']
        peaksnr[pulse]['Time'] = z[rownum + 1]['Time']
...
if z[rownum]['SNR'] > peaksnr[pulse]['Peak SNR']:
    peaksnr[pulse]['Peak SNR'] = z[rownum]['SNR']
    peaksnr[pulse]['Time'] = z[rownum]['Time']
    
    
print(np.sum(z['SNR']))
print(np.sum(peaksnr['Peak SNR']))

#print(peaksnr)

'''plt.plot(peaksnr['Pulse'], peaksnr['Peak SNR'], 'ro')
plt.show()'''
...
while rownum < numpul:
    bin = 0
    while bin < numbin:
        if 10 + binsize*bin <= peaksnr[rownum]['Peak SNR']:
            binarray[bin] += 1
        bin += 1
    rownum += 1
...
#Plot the log-log plot with the line of best fit
